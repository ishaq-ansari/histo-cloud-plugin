...
def _max_clustering_cython(double[:, :] im not None, int[:, :] im_fgnd_mask not None, double rad):
    ...
    cdef double[:, ::1] local_max_val = np.zeros([sy, sx], dtype=np.float64)
    cdef long[:, ::1] local_max_ind = np.zeros([sy, sx], dtype=np.intp)
    ...
    cdef cnp.ndarray[cnp.intp_t, ndim=2, mode='c'] maxpath_np = np.zeros([1000, 2], dtype=np.intp)
